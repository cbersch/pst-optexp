...
\newcommand{\objdesc}[1]{\textit{Obj}\ifx#1\relax\else$_{#1}$\fi\xspace}
...
\subsubsection{Naming Objects}\label{sec:namingobj}
\begin{fieldlist}
  \fielditem{compname}{<string>}{} 

  Every \LPack{pst-optexp} object of an
  experimental setup can be assigned a name that is unique within one
  \nxLkeyword{pspicture} environment. The name is defined with the parameter
  \Lkeyword{compname} which is defineable only directly within a
  \LPack{pst-optexp} object.
...
\begin{BDef}
  \Lcs{oenode}\Largb{Obj}\Largb{Node}
\end{BDef}
...
The basic macro is \nxLcs{oenode} which takes as first argument the name of the
...
\begin{BDef}
  \Lcs{oenoderefa}\Largb{Obj}\\
  \Lcs{oenoderefb}\Largb{Obj}
\end{BDef}

\nxLcs{oenoderefa} and \nxLcs{oenoderefb} refer to the reference nodes which are used to position \objdesc{}. For the dipole components 

\begin{LTXexample}
\begin{pspicture}(3,2)
  \pnode(0,1){A}\pnode(3,1){B}\lens(A)(B){}
...
\begin{BDef}
  \Lcs{drawbeam}\OptArgs\Largb{Obj$_1$}\Largb{Obj$_2$}\ldots\Largb{Obj$_N$}\\[3mm]
%\end{BDef}
%\begin{BDef}
...
\fielditem{startpos}{<num> [<num>]}{0 0}
...
\fielditem{startvec}{<num> [<num>]}{1 0} This is the start vector of the beam.
\medskip
