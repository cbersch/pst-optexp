...
\newenvironment*{fieldlist}
  {\list{}{%
     ...
     \renewcommand*{\makelabel}[1]{\hss\spotcolor\marglistfont##1}}%
   \def\choiceitem##1##2##3{%
     \item[##1]%
     \xLkeyword{##1}%
     ##2 \ifx##3\relax\else (default: \texttt{##3})\fi\par\nobreak
     \vspace{\itemsep}}%
   \def\fielditem##1##2##3{\choiceitem{##1}{\prm{##2}}{##3}}
   \def\styleitem##1##2{%
     \item[##1]%
     \xLstyle{##1}\xLkeyword{addto##1}\xLkeyword{new##1}%
     \prm{psstyle} \ifx##2\relax\else (default: \texttt{##2})\fi\par\nobreak
     \vspace{\itemsep}}%
...
\begin{fieldlist}
\choiceitem{namingscheme}{old|new}{new}
...
\choiceitem{labelref}{relative|relgrav|global}{relgrav}
...
\choiceitem{label}{\prm{offset}[ \prm{angle}[ \prm{ref string}[ \prm{labelref}]]]}{}
...
\subsection{Accessing Objects and Related Nodes}\label{sec:objnodes}
...
All objects are numbered automatically in increasing order by their definition in the code. The objects as well as their special nodes can be accessed via this number.

\begin{fieldlist}
  \fielditem{compname}{string}{} This parameter can be used to assign a unique
...
\begin{optionlist}
\valitem[1]{lensheight}{num} Set the height of the lens.
...
\valitem[1]{lensradiusleft}{num} Set the left radius of the lens. A positive
\prm{num} is for convex, a negative one for concave curvatures. Use zero for a plain
surface.

\valitem[1]{lensradiusright}{num} Same as \opt{lensradiusleft} but for the right surface.
...
\valitem{lensradius}{num} A convenience option to specify both curvatures.
...
\begin{optionlist}
  \valitem[1]{plateheight}{num} The height of the plate.
...
\valitem[0]{angle}{num} Rotate the plate around its center.
...
\begin{optionlist}
\valitem[1]{plateheight}{num} The height of the plate.
\valitem[0.1]{platewidth}{num} The width of the plate.
...
\begin{optionlist}
\valitem[1]{outerheight}{num}
\valitem[0.1]{innerheight}{num}
...
\fielditem{rotateref}{ref string}{c}
...
\choiceitem{dettype}{round|diode}{round}
...
\begin{optionlist}
\optitem[parallel]{poltype}{parallel, perp, misc, lcirc, rcirc}
\valitem[0.6]{polsize}{num}
...
\valitem[0]{mirrorradius}{num} This parameter defines the curvature of the
mirror. A value of \opt{0} is for a plain mirror, a negative radius for a
concave mirror and a positive radius gives you a convex mirror.
...
\begin{pspicture}(3,3)
  ...
  \drawbeam[connectplanes]{(0,0)}{1}{2}{(2.5,3)}
\end{pspicture}
\end{LTXexample}

\optitem[normal]{mirrortype}{normal, piezo, extended}
...
\begin{LTXexample}
  \begin{pspicture}(3,2)
  \psset{beam}
  \mirror[mirrortype=extended](0,0)(1.8,1.5)(0,2){ext}
\end{pspicture}
\end{LTXexample}
\medskip
\begin{LTXexample}
  \begin{pspicture}(3,2)
  \psset{beam}
  \mirror[mirrortype=piezo](0,0)(1.8,1.5)(0,2){piezo}
\end{pspicture}
\end{LTXexample}

\valitem{ExtendedMirror}{psstyle} This style defines the appearance of the extended mirror.
\begin{LTXexample}
\begin{pspicture}(3,2)
  \psset{beam}
...
\valitem{PiezoMirror}{psstyle} This style defines the appearance of the piezo
...
\valitem{VariableMirror}{psstyle} With the \opt{variable} option a moveable
...
\valitem[0.1]{mirrordepth}{num} Define the depth of an extended mirror
\begin{LTXexample}
\begin{pspicture}(3,2)
  \psset{mirrortype=extended, mirrordepth=0.2}
  \mirror[beam](0,0)(1.8,1.5)(0,2){}
\end{pspicture}
\end{LTXexample}
\end{optionlist}
...
\optitem[cube]{bsstyle}{cube, plate} This parameter selects between different
...
\end{ltxsyntax}
\begin{optionlist}
\valitem[1]{optgridwidth}{num} The width of the grating.
\begin{LTXexample}
\begin{pspicture}(3,2)
...
\valitem[0.15]{optgridheight}{num} This is the total height of the grating.

\valitem[0.075]{optgriddepth}{num} This is the modulation depth of the grating structure. The total height is unaffected by this value.
\begin{LTXexample}
\begin{pspicture}(3,2)
  \optgrid[beam, 
             optgriddepth=0.05](0,2)(1.8,1.5)(0,0){grid}
\end{pspicture}
\end{LTXexample}
...
\begin{pspicture}(3,2)
  \optgrid[beam, 
             optgridcount=3](0,2)(1.8,1.5)(0,0){grid}
\end{pspicture}
\end{LTXexample}

\optitem[blazed]{optgridtype}{blazed, binary} Select between a \opt{binary} and a \opt{blazed} grating.
\begin{LTXexample}
\begin{pspicture}(3,2)
  \optgrid[beam, 
             optgridtype=binary](0,2)(1.8,1.5)(0,0){grid}
\end{pspicture}
\end{LTXexample}
...
The prism is always placed symmetric between \prm{In} and \prm{Out} nodes. For asymmetric beam traces see \prettyref{sec:connecting}.

\begin{optionlist}
\valitem[1]{prismsize}{num} This is the height of the prism.
\begin{LTXexample}
\begin{pspicture}(3,3)
  \optprism[beam](0,2.5)(2,2)(3,0){Prism}
\end{pspicture}
\end{LTXexample}

\valitem[60]{prismangle}{num} 
\begin{LTXexample}
\begin{pspicture}(3,3)
  \optprism[beam, prismangle=30](0,2.5)(2,2)(3,0){Prism}
\end{pspicture}
\end{LTXexample}
\end{optionlist}
...
\begin{optionlist}
\valitem[1.5]{raprismsize}{num} This is the length of the input plane.
\begin{LTXexample}
\begin{pspicture}(3,2)
  \psdot(1.8,1)
...
\medskip In the next example the \prm{Center} node is displaced vertically:
...
\begin{optionlist}
  \valitem[0.7]{pentaprismsize}{num} This is the length of the input and output plane.
\begin{LTXexample}
\begin{pspicture}(3,2)
...
\valitem[0.4]{fiberloopradius}{num}
\begin{LTXexample}
\begin{pspicture}(3,1.5)
  \psset{labeloffset=0.3}
...
\begin{optionlist}
\valitem[0.8]{optampsize}{num} The length of all triangle sides.
\begin{LTXexample}
\begin{pspicture}(3,1.5)
  \optamp(0,1)(3,1){EDFA}
\end{pspicture}
...
\begin{optionlist}
\valitem[0.8]{optmzmsize}{num} This is the height of the component, the width
...
\begin{optionlist}
\valitem[0.8]{filtersize}{num}
\optitem[bandpass]{filtertype}{bandpass, bandstop, lowpass, highpass}
\begin{LTXexample}
\begin{pspicture}(3,1.5)
...
\begin{optionlist}
\valitem[0.15]{polcontrolsize}{num} The radius of the polarization controller circles.
\begin{LTXexample}
\begin{pspicture}(3,1.5)
...
\begin{optionlist}
\valitem[0.6]{isolatorsize}{num} The height of the isolator. The width is \opt{1.6} times this value. Use the \opt{unit} parameter to change this relation
\begin{LTXexample}
\begin{pspicture}(3,1.5)
...
\valitem{IsolatorArrow}{psstyle} This changes the appearance of the isolator
...
\valitem[opened]{switchstyle}{opened, closed} Indicate the switch as \opt{opened} or \opt{closed}
\begin{LTXexample}
\begin{pspicture}(3,1.5)
...
\valitem{FdlArrow}{psstyle} This changes the appearance of the delay line arrow
...
\end{ltxsyntax}
\begin{optionlist}
\valitem[0.3]{fibercolsize}{num} The length of every side.
\end{optionlist}

The \nxLcs{fibercollimator} can be used with two, three or four points. With two points, the collimator is placed like any other dipole component between \prm{In} and \prm{Out} node:
...
\begin{pspicture}(3,1.5)
  ...
  \fibercollimator[beam](0.5,1)(2.5,1){FC}
\end{pspicture}
\end{LTXexample}

\medskip

For three nodes, the fiber is drawn as \Lcs{psbezier} curve for which the
...
\begin{LTXexample}
\begin{pspicture}(3,2)
   \fibercollimator[position=0.2,
                      beam](0.5,1)(2.5,1)(2.5,2)(0.5,2){FC}
\end{pspicture}
\end{LTXexample}
...
\begin{optionlist}
\valitem[0.2]{couplersize}{num}
\valitem[0.1]{couplersep}{num}
\optitem[elliptic]{couplertype}{none, elliptic, rectangular, crossswitch}
\optitem[center]{align}{top, bottom, center}
\end{optionlist}
...
\begin{LTXexample}[width=3.5cm]
\begin{pspicture}[showgrid=true](3,2)
  \optcoupler[align=bottom, couplertype=rectangular](0.5,2)(0,0.5)(3,1.5)(2.5,0){Coupler}
\end{pspicture}
\end{LTXexample}
...
\styleitem{FiberIn1}{style=FiberIn}
\styleitem{FiberIn2}{style=FiberIn}
...
\styleitem{FiberOut}{style=Fiber}
\styleitem{FiberOut1}{style=FiberOut}
\styleitem{FiberOut2}{style=FiberOut}
...
\choiceitem{startpos}{\prm{num} [\prm{num}]}{0}
...
\choiceitem{startvec}{\prm{num} [\prm{num}]}{1} This is the start vector of the beam.
\medskip
...
\fielditem{startvecabs}{boolean}{false}

\fielditem{connectplanes}{boolean}{false}

\fielditem{n}{num}{1.5}

\fielditem{beaminside}{boolean}{false}

\fielditem{beaminsidelast}{boolean}{false}

\fielditem{beaminsidefirst}{boolean}{false}
\end{fieldlist}

\subsection{Drawing wide beams}\label{sec:drawwidebeam}

\begin{fieldlist}
\fielditem{beamwidth}{num}{0.1}

\fielditem{beamdiv}{num}{0}
\end{fieldlist}
...
\begin{fieldlist}
\fielditem{startinside}{boolean}{false}

\fielditem{stopinside}{boolean}{false}
\end{fieldlist}
% \begin{description}
