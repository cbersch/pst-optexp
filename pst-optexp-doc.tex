\documentclass[%
11pt, english, DIV12, parskip=false, headings=small,
headinclude=false,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage {etoolbox}
\AtBeginEnvironment {lstlisting}% if you use lstlistings
            {\let\lsthk@OutputBox \lsthk@OutputBox@H@@k }
%\AtBeginDocument{\preto\FV@VerbatimBegin {\let\lsthk@OutputBox 
%\lsthk@OutputBox@H@@k }}
\providerobustcmd*\ifrefundefined [1]{\begingroup \csname 
@safe@activestrue \endcsname %<babel>
     \expandafter\endgroup \csname @\ifcsname r@#1\endcsname second\else
                                                             first\fi 
oftwo\endcsname
}% \ifrefundefined
\def\lsthk@OutputBox@H@@k {\begingroup \let\lst@UM \@empty
     \edef \@tempc {sec:\detokenize\expandafter{\the\lst@token }}%\edef 
\@tempc {\@tempc }
     \ifrefundefined {\@tempc}   \endgroup \sethyperlistings
...
\def\hyperlistings {\setbox \@tempboxa
     \hbox   \bgroup
             \rlap {\hypersetup {linkcolor=.}\relax  \fboxrule \z@
             \hyperref [\hyperlistingsreference]
                     {\boxframe 
{\wd\@tempboxa}{\ht\@tempboxa}{\dp\@tempboxa}}}%
             \unhbox \@tempboxa
             \egroup
}% \hyperlistings
% -----------------------------------------------------------
...
\newcommand{\parampar}[1]{\paragraph{\Lkeyword{#1}}}
...
\begin{LTXexample}[width=3.5cm]
\begin{pspicture}[showgrid=true](3,2) 
  \lens[beam, startvecabsolute, startvec=1 0, compshift=0.1](0,1.2)(3,1.2){L}
\end{pspicture}
\end{LTXexample}
...
\begin{LTXexample}[width=3.5cm]
\begin{pspicture}[showgrid=true](3,2) 
  \mirror[beam, compshift=0.2, startvecabsolute, startvec=1 0](0,1)(2,1)(2,0){mirror}
\end{pspicture}
\end{LTXexample}
...
\parampar{labeloffset} Specify the offset from the label 
reference node of the object which in most cases is the center. 

\parampar{labelstyle} Define the textstyle that is used to typeset 
the label. 

\parampar{labelalign} Has the same meaning as the refpoint of 
\Lcs{rput} and can take the same values. 

\parampar{labelangle} Set the label rotation angle around the
object. \Lkeyword{labelref} can be used to change the reference system.

\parampar{labelref} Set the reference coordinate system for the \Lkeyword{labelangle} and the orientation of
...
\parampar{label} Allows simplified notation for changing more than one
...
\begin{BDef}
  \Lcs{drawbeam}\OptArgs\Largb{$C_1$}\Largb{$C_2$}\ldots\Largb{$C_N$}
\end{BDef}
...
\begin{BDef}
  \Lcs{drawwidebeam}\OptArgs\Largb{$C_1$}\Largb{$C_2$}\ldots\Largb{$C_N$}
\end{BDef}
