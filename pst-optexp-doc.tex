...
\begin{pspicture}[showgrid=false](-0.2,0.1)(8.2,5.3)
  ...
  \newpsstyle{Beam}{}
  \psline[ArrowInside=->, linewidth=3\pslinewidth, arrowinset=0](In)(\OENodeIn{@3})(\OENodeOut{@4})
  \drawbeam[startvec=0.012,startpos=0.015, addtoBeam={arrows=<-, linewidth=3\pslinewidth, arrowinset=0}]{([Xnodesep=-0.2]Out)}{8}{7}.
\end{optexp}
\rput(4,4.5){%
...
\Lkeyword{compat} package option allows you to compile old setups but without
profiting from the new features.
...
\noindent\Lkeyword{optional} can be used with every object and marks it as
optional. The style of an optional element can be configured by changing
the psstyle \Lkeyword{OptionalStyle}.

\Lkeyword{showoptdots} draws some internal nodes which are used to
...
\noindent\Lkeyword{OptComp} affects only the appearence of the optical
...
\noindent\Lkeyword{position} is equivalent to the \Lkeyword{npos}
parameter of \Lcs{ncput} (can be any number from 0 to 1) and controls the
...
\noindent\Lkeyword{labeloffset} specifies the offset from the label 
reference node of the object which is mostly the center. 
\Lkeyword{labelstyle} defines the textstyle that is used to typeset 
the label and \Lkeyword{labelalign} corresponds to the refpoint of 
\Lcs{rput}. The parameter \Lkeyword{labelref} sets the reference 
coordinate system for the \Lkeyword{labelangle} and the orientation of
...
\Lkeyword{label} simplifies the simultaneous change of more than one label-related parameter. It takes up to four space-separated arguments. Unchanged arguments may be specified with a dot.
...
\noindent Every \LPack{pst-optexp} object of an experimental setup can
...
If \Lkeyword{compname} is empty, the external node has the name
...
\Lkeyword{extnode} controls the position of the additional node and
...
(see Sec.~\ref{sec:namedobj} for naming conventions). If \Lkeyword{compname} is empty
...
\begin{pspicture}[showgrid=true](5,2) 
   ...
   \custombeam{%
     \drawbeam[beamwidth=0.1]{(A)}{1}.
     \drawbeam[beamdiv=1]{1}{2}.
   }
\end{pspicture}
\end{LTXexample}
\medskip
...
\LPack{pst-optexp} version 2.1 requires at least version 2.87 of
\LPack{pstricks-add} and \LaTeX. It does not work with plain \TeX.
