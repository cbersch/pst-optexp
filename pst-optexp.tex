...
%
\edef\pst@string@coupler@type@free{free}
...
\newpsstyle{FiberIn1}{ArrowInside=->,linecolor=green}%
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\define@key[psset]{optexp}{optmzmheight}{\edef\psk@optexp@optmzm@height{#1}}
\define@key[psset]{optexp}{optmzmwidth}{\edef\psk@optexp@optmzm@width{#1}}
%
% Isolator
...
\ignorespaces}%
%
% Equivalent to new@optexpdipole for fiber dipole objects.
   ...
   \@ifundefined{#2@i}{%
      ...
      \expandafter\def\csname #2@i\endcsname(##1)(##2)##3{%
         ...
         \begingroup
            \use@par
            \pst@regNodes{##1}{##2}
            \pst@draw@fibercomponent{##3}{\@nameuse{#2@ii}}
            % 
            % connect the fibers
            \bgroup
               \@ifundefined{pscs@FiberIn}{\psset{style=Fiber}}{\psset{style=FiberIn}}%
               \pcline(tempNode@A)(tempNode@A@)
            \egroup
            \bgroup
               \@ifundefined{pscs@FiberOut}{\psset{style=Fiber}}{\psset{style=FiberOut}}%
               \pcline(tempNode@B)(tempNode@B@)
            \egroup
         \endgroup
      }%
      \@namedef{#2@ii}{%
   ...
   \@ifundefined{#2@i}{%
      ...
      \expandafter\def\csname #2@i\endcsname(##1)(##2)(##3)(##4)##5{%
         ...
         \begingroup
            ...
            \pst@draw@fibercomponent{##5}{\@nameuse{#2@ii}}
            % 
            % connect the fibers
            \bgroup
               \@ifundefined{pscs@FiberIn1}{\psset{style=Fiber}}{\psset{style=FiberIn1}}%
               \pccurve[angleB=! \pst@optexpdict FiberQuadAngleB end,
                        angleA=! \pst@optexpdict FiberQuadAngleA end](##1)(tempNode@A@1)
            \egroup
            \bgroup
               \@ifundefined{pscs@FiberIn2}{\psset{style=Fiber}}{\psset{style=FiberIn2}}%
               \pccurve[angleB=! \pst@optexpdict FiberQuadAngleB end,
                        angleA=! \pst@optexpdict FiberQuadAngleA end](##2)(tempNode@A@2)
            \egroup
            \bgroup
               \@ifundefined{pscs@FiberOut1}{\psset{style=Fiber}}{\psset{style=FiberOut1}}%
               \pccurve[angleA=! \pst@optexpdict FiberQuadAngleA end,
                        angleB=! \pst@optexpdict FiberQuadAngleB end](tempNode@B@1)(##3)
            \egroup
            \bgroup
               \@ifundefined{pscs@FiberOut2}{\psset{style=Fiber}}{\psset{style=FiberOut2}}%
               \pccurve[angleA=! \pst@optexpdict FiberQuadAngleA end,
                        angleB=! \pst@optexpdict FiberQuadAngleB end](tempNode@B@2)(##4)
            \egroup
         \endgroup
      }%
      \@namedef{#2@ii}{%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DRAW FIBER COMPONENTS
%
% This macro is called by every fiber unit
%
\def\pst@draw@fibercomponent#1#2{%
    %
    % linestyle to use, if component should be marked as optional
    \ifPst@optexp@component@optional
      \psset{style=OptionalStyle}
    \fi
    \ncline[linestyle=none]{tempNode@A}{tempNode@B}%
    \ncput[nrot=:U,npos=]{\put@Comp{#2}}
    %
    % Now put the label
    \put@Label{#1}%
...
\def\optbox@iii{%
   ...
   \ifPst@optexp@endbox
      ...
      \fi
   \else
      \pcline[style=Fiber](tempNode@A)(! \@b@wd neg 0)
      \pcline[style=Fiber](tempNode@B)(! \@b@wd 0)
...
%
\def\optmzm@iii{%
   \edef\@wd{\psk@optexp@optmzm@width\space}%
   \edef\@ht{\psk@optexp@optmzm@height\space}%
   \bgroup
      \psset{style=Fiber}
...
%
\def\optamp@iii{%
   \def\getCLW{CLW \pst@number\psxunit 2 mul div\space}%
   \edef\@sz{\psk@optexp@optamp@size\space 0.5 mul\space}%
   \edef\@xl{0.75 sqrt \@sz mul\space}%
   \pspolygon(! \@xl 0)(! \getCLW \@xl sub \@sz)(! \getCLW \@xl sub \@sz neg)
...
\def\optcoupler@iii{%
   ...
   \ifx\psk@optexp@coupler@type\pst@string@coupler@type@elliptic
      ...
      \fi
      \pnode(! \@sz neg \@sz 0.05 mul){tempNode@A@1}
      \pnode(! \@sz neg \@sz -0.05 mul){tempNode@A@2}
      \pnode(! \@sz \@sz 0.05 mul){tempNode@B@1}
      \pnode(! \@sz \@sz -0.05 mul){tempNode@B@2}
   \fi
   \ifx\psk@optexp@coupler@type\pst@string@coupler@type@free
      \pnode(! 0 \@sz 0.1 mul){tempNode@A@1}
      \pnode(! 0 \@sz -0.1 mul){tempNode@A@2}
      \pnode(! 0 \@sz 0.1 mul){tempNode@B@1}
      \pnode(! 0 \@sz -0.1 mul){tempNode@B@2}
   \fi
}%
%
%
