...
\define@key[psset]{optexp}{nAdd}{\edef\POK@refractiveindex@add{#1}}
%
% Plate
...
\def\drawbeam@iii{%
  \ifx\pslinestyle\@none\else
    \addto@pscode{%
     {/nMul \POK@refractiveindex@mul\space def
      /nAdd \POK@refractiveindex@add\space def} }%
...
\def\optexp@getnode#1\@nil{%
  ...
  \else
    \ifcat\pst@tempb a\relax
      \edef\oe@temp{/\POS@optexp@basicname@prefix#1\space}%
    \else
      \edef\oe@temp{/\POS@optexp@basicname@prefix\POS@optexp@basicname@default#1\space}%
    \fi
  \fi%
}%
...
\def\drawfiber@i#1#2{%
  ...
  \begingroup
    ...
    \fi
    %
    \message{--\meaning\psk@angleA--}
    \message{^^Joptexp@comps --\optexp@comps--^^J}
    \ifx\relax\psk@angleA
      \message{angleA relax}
      \psset{angleA=! \pst@optexpdict \optexp@comps RelFiberAngle end}%
    \else
      \ifPOE@relangle
        \psset{angleA=! \pst@optexpdict \optexp@comps RelFiberAngle end \psk@angleA\space add}%
    \fi\fi
    \ifx\psk@angleB\relax
      \psset{angleB=! \pst@optexpdict \optexp@comps exch RelFiberAngle end}%
    \else
      \ifPOE@relangle
        \psset{angleB=! \pst@optexpdict \optexp@comps exch RelFiberAngle end \psk@angleB\space add}%
    \fi\fi
    \pccurve(! \optexp@comps exch \pst@optexpdict NearestNode end \tx@UserCoor)%
...
\def\newOptexpPlane(#1)(#2)#3{%
    ...
    \pst@Verb{\pst@optexpdict {\pst@tempOrig} gsave STV CP T exec grestore ToVec {\pst@tempDiff} (\POS@optexp@basicname@prefix#3) {} NewNodeComp end}%
\ignorespaces}%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% END %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\def\optexp@pccurve@i[#1]{\optexp@pccurve@[#1]{(\POK@basicname) dup }}%
\def\optexp@pccurve@{\pst@object{optexp@pccurve@}}%
\def\optexp@pccurve@@i#1{%
  \begingroup
    \use@par
    % \ifPOE@relangle
    %   \psset{angleA=! \pst@optexpdict #1 RelFiberAngleA end \psk@angleA\space add,
    %          angleB=! \pst@optexpdict #1 RelFiberAngleB end \psk@angleA\space add}
    % \fi
    \pccurve(! #1 exch \pst@optexpdict NearestNode end \tx@UserCoor)%
            (! #1 \pst@optexpdict NearestNode end \tx@UserCoor)
  \endgroup
}%
% \def\optexp@pccurve{\@ifnextchar[{\optexp@pccurve@i}{\optexp@pccurve@i[]}}
