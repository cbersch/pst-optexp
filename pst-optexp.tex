...
\def\optexp@beamnodes#1#2#3#4#5{%
   \pst@Verb{%
      tx@NodeDict begin
      #1 \pst@number\psxunit mul
      #2 \pst@number\psyunit mul
      #3 \pst@number\psxunit mul
      #4 \pst@number\psyunit mul
      \pst@optexpdict ExchCoorSwitch end
      { % exchange coordinates
        neg exch neg exch [ 3 1 roll ] cvx false /N@#5B 10 {InitPnode } NewNode
        neg exch neg exch [ 3 1 roll ] cvx false /N@#5T 10 {InitPnode } NewNode
      }{ 
        [ 3 1 roll ] cvx false /N@#5B 10 {InitPnode } NewNode
        [ 3 1 roll ] cvx false /N@#5T 10 {InitPnode } NewNode
      } ifelse
      end
    }%
}%
...
% COUPLER
%
\def\optcrossswitch@iii{%
   \edef\@wd{\POK@crossswitch@width\space 0.5 mul\space}%
   \edef\@ht{\POK@crossswitch@height\space 0.5 mul\space}%
   \edef\@sep{\POK@crossswitch@sep\space 0.5 mul\space}%
   \ifx\POK@align\POS@optexp@top
      \pst@Verb{/@yshift \@sep neg def}%
   \else\ifx\POK@align\POS@optexp@bottom
      \pst@Verb{/@yshift \@sep def}%
   \else
      \pst@Verb{/@yshift 0 def}%
   \fi\fi
   \psframe[dimen=outer](!\@wd neg @yshift \@ht sub )(! \@wd @yshift \@ht add)
   \ifdim\POK@crossswitch@width pt<\POK@crossswitch@height pt
      \edef\@csz{\POK@crossswitch@width\space 0.3 mul\space}
   \else
      \edef\@csz{\POK@crossswitch@height\space 0.3 mul\space}
   \fi
   \psline(! \@csz neg dup)(! \@csz dup)
   \psline(! \@csz neg \@csz)(! \@csz dup neg)
   \pnode(! \@wd neg @yshift \@sep add){tempNode@A@1}
   \pnode(! \@wd neg @yshift \@sep sub){tempNode@A@2}
   \pnode(! \@wd @yshift \@sep add){tempNode@B@1}
   \pnode(! \@wd @yshift \@sep sub){tempNode@B@2}
   \pnode(! 0 @yshift){tempNode@Label}
}%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
