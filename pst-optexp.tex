...
\define@boolkey[psset]{optexp}[POE@]{startDrawInside}[true]{}%
\define@boolkey[psset]{optexp}[POE@]{stopDrawInside}[true]{}%
...
\define@boolkey[psset]{optexp}[POE@]{beamContinue}[true]{}%
...
\def\drawbeam@i{%
  ...
  \addto@pscode{%
     \pst@optexpdict 
     1 setlinejoin
     /startDrawInside \ifPOE@startDrawInside true \else false \fi def
     /stopDrawInside \ifPOE@stopDrawInside true \else false \fi def
     /beamInside \ifPOE@beamInside true \else false \fi def
     /continueBeam \ifPOE@beamContinue true \else false \fi def
...
\def\drawbeam@iii{%
  \ifx\pslinestyle\@none\else
    ...
    \ifx\POK@startpos\@empty
      \addto@pscode{ {0 0} }%
    \else
      \addto@pscode{ {\POK@startpos \tx@ScreenCoor} }%
    \fi
    \addto@pscode{%
       {\ifx\POK@startvec\@empty 0 0 \else \POK@startvec\fi} 
       gsave 
...
\def\drawExtendedBeam@iii{%
  ...
  \addto@pscode{%
   {/nMul \POK@nmul\space def
    /nAdd \POK@nadd\space def} }%
  % the input vectors
  \ifx\POK@startvec\@empty
     \ifx\POK@beamdiv\@empty
        \addto@pscode{ {0 0} dup }%
     \else
        \addto@pscode{ {\POK@beamdiv\space dup cos exch sin } {\POK@beamdiv\space dup cos exch sin neg } }%
     \fi
  \else
     \addto@pscode{ {\POK@startvec} dup }%
  \fi
  % the start positions
  \ifx\POK@beamwidth\@empty
     \addto@pscode{ {0 0} dup }%
  \else
     \addto@pscode{ {0 \POK@beamwidth\space 2 div \tx@ScreenCoor} {0 \POK@beamwidth\space -2 div \tx@ScreenCoor} }%
  \fi
  % rearrange options
...
%        \pnode(0,0){\optexp@nodeOut}%
        \@ifundefined{#2@iii}{%
          \ifPOE@bglayer
            \@ifundefined{#2@nodes}{\dipole@nodes}{\@nameuse{#2@nodes}}%
          \fi
          \ifPOE@toplayer
            \@nameuse{#2@comp}%
          \fi
...
%        \pnode(0,0){\optexp@nodeOut}%
        \@ifundefined{#2@iii}{%
          \ifPOE@bglayer
            \@ifundefined{#2@nodes}{\tripole@nodes}{\@nameuse{#2@nodes}}%
          \fi
          \ifPOE@toplayer
            \@nameuse{#2@comp}%
          \fi
...
%
\def\optisolator@iii{%
   \edef\@ht{\POK@isolatorsize\space 2 div\space}%
...
\def\optcoupler@iii{%
   ...
   \ifx\POK@couplertype\POS@couplertype@none
      \pnode(! \@sz -0.5 mul @yshift \@sep add){\POK@b@sicname 1}
      \pnode(! \@sz -0.5 mul @yshift \@sep sub){\POK@b@sicname 2}
      \pnode(! \@sz 0.5 mul @yshift \@sep add){\POK@b@sicname 3}
      \pnode(! \@sz 0.5 mul @yshift \@sep sub){\POK@b@sicname N}
   ...
   \fi\fi
      ...
      \pnode(! \@sz \getCLW 0.3 mul sub @yshift \@sep add){\POK@b@sicname 3}
      \pnode(! \@sz \getCLW 0.3 mul sub @yshift \@sep sub){\POK@b@sicname N}
...
\def\wdmsplitter@ii{%
   ...
   \ifx\POK@couplertype\POS@couplertype@elliptic
      \psellipse[dimen=outer](! 0 @yshift)(! \@sz \@sz 0.4 mul)
      \pnode(! \@sz neg \getCLW 0.3 mul add 0){\POK@b@sicname 1}
      \pnode(! \@sz \getCLW 0.3 mul sub @yshift \@sep add){\POK@b@sicname 2}
      \pnode(! \@sz \getCLW 0.3 mul sub @yshift \@sep sub){\POK@b@sicname N}
      \pnode(! 0 @yshift){\optexp@nodeLabel}
   \else\ifx\POK@couplertype\POS@couplertype@none
      \pnode(0,0){\POK@b@sicname 1}
