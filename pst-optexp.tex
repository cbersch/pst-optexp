...
\def\wdmsplitter@i(#1)(#2)(#3)#4{%
   ...
   \begin@OptexpObj
      ...
      % connect the fibers
      \ifPOE@fiberin@
        \optexp@nccurve(#1)(\POK@basicname 1){FiberIn}
      \fi
      \ifPOE@fiberout@top
        \optexp@nccurve(\POK@basicname 2)(#2){FiberOut1}
      \fi
      \ifPOE@fiberout@bottom
        \optexp@nccurve(\POK@basicname N)(#3){FiberOut2}
      \fi
   \end@OptexpObj
}%
\def\optexp@nccurve{\pst@object{optexp@nccurve}}%
\def\optexp@nccurve@i(#1)(#2)#3{%
  \addbefore@par{style=#3}%
  \begingroup
    \edef\pst@tempa{\psk@angleA}
    \edef\pst@tempb{\psk@angleB}
    \let\psk@angleA\relax
    \let\psk@angleB\relax
    \use@par
    \ifPOE@relangle
      \ifx\psk@angleA\relax\else
        \edef\pst@tempa\psk@angleA
      \fi
      \ifx\psk@angleB\relax\else
        \edef\pst@tempb\psk@angleB
      \fi
%      \psset{angleA=! \pst@optexpdict N@\optexp@nodeB\space N@\optexp@nodeA\space FiberAngleA end \pst@tempa\space add,%
%             angleB=! \pst@optexpdict N@\optexp@nodeB\space N@\optexp@nodeA\space FiberAngleB end \pst@tempb\space add}%
%    \else
