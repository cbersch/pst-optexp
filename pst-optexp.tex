...
\def\optexp@getnode#1\@nil{%
  \edef\pst@tempb{\pst@optexp@getfirstchar#1\@nil}%
  \def\pst@tempc{(}
  \message{^^J----\pst@tempb----^^J}
  \ifx\pst@tempb\pst@tempc\relax
    \message{getcoor, #1^^J^^J}
    \optexp@getcoor#1%
  \else\ifcat\pst@tempb a\relax
    \message{getcomp, #1^^J^^J}
    \optexp@getcomp{(#1)}%
  \else
    \message{getcomp temp, #1^^J^^J}
...
%  \edef\poe@tmp{\the\poe@cnt}% I don't know how to do it with expansion
  ...
  \else
    \ifPOE@fiberin@
      \optexp@pccurve(\optexp@nodeA)(\POK@basicname 1){FiberIn}
    \fi
    \ifPOE@fiberout@
      \optexp@pccurve(\POK@basicname N)(\optexp@nodeB){FiberOut1}
    \fi
  \fi
}%
\define@choicekey[psset]{optexp}{fiber}[\val\nr]{}[]{%
   ...
   \@ifundefined{#2@i}{%
      ...
      \expandafter\def\csname #2@i\endcsname(##1)(##2)(##3)(##4)##5{%
         ...
         \begin@OptexpObj
            ...
            % connect the fibers
            \ifPOE@fiberin@top
              \optexp@pccurve(##1)(\POK@basicname 1){FiberIn1}
            \fi
            \ifPOE@fiberin@bottom
              \optexp@pccurve(##2)(\POK@basicname 2){FiberIn2}
            \fi
            \ifPOE@fiberout@top
              \optexp@pccurve(\POK@basicname 3)(##3){FiberOut1}
            \fi
            \ifPOE@fiberout@bottom
              \optexp@pccurve(\POK@basicname N)(##4){FiberOut2}
            \fi
         \end@OptexpObj
      }%
      \@namedef{#2@ii}{%
...
\def\wdmsplitter@i(#1)(#2)(#3)#4{%
   ...
   \begin@OptexpObj
      ...
      % connect the fibers
      \ifPOE@fiberin@
        \optexp@pccurve(#1)(\POK@basicname 1){FiberIn}
      \fi
      \ifPOE@fiberout@top
        \optexp@pccurve(\POK@basicname 2)(#2){FiberOut1}
      \fi
      \ifPOE@fiberout@bottom
        \optexp@pccurve(\POK@basicname N)(#3){FiberOut2}
      \fi
   \end@OptexpObj
}%
\def\optexp@pccurve{\pst@object{optexp@pccurve}}%
\def\optexp@pccurve@i(#1)(#2)#3{%
  ...
  \begingroup
    ...
    \else\ifPOE@relangle
        \psset{angleA=! \pst@optexpdict N@\optexp@nodeB\space N@\optexp@nodeA\space FiberAngleA end \psk@angleA\space add}%
    \fi\fi
    \ifx\psk@angleB\relax
      % angleA was not changed by the style
      \psset{angleB=! \pst@optexpdict N@\optexp@nodeB\space N@\optexp@nodeA\space FiberAngleB end}%
    \else\ifPOE@relangle
        \psset{angleB=! \pst@optexpdict N@\optexp@nodeB\space N@\optexp@nodeA\space FiberAngleB end \psk@angleB\space add}%
    \fi\fi
    \pccurve(#1)(#2)
  \endgroup
}%
