...
\newif\ifPst@oe@startvecAbsolute
\newif\ifPst@oe@beamInside
\newif\ifPst@oe@beamextended
\newif\ifPst@oe@beamConnectComp
...
\def\optexp@getnode#1\@nil{%
  ...
  \else
    \ifcat\pst@tempb a\relax
      \edef\oe@temp{/\pst@str@basicname@prefix#1\space}%
    \else
      \edef\oe@temp{/\pst@str@basicname@prefix\pst@str@basicname@default#1\space}%
    \fi
  \fi%
}%
...
\define@boolkey[psset]{optexp}[Pst@oe@]{beamextended}[true]{}%
\define@boolkey[psset]{optexp}[Pst@oe@]{beamConnectComp}[true]{}%
\define@boolkey[psset]{optexp}[Pst@oe@]{startvecAbsolute}[true]{}%
%
...
\def\custombeam@i#1{%
   ...
   \let\@oldsinglebegin\begin@SingleBeam
   \def\begin@SingleBeam{\message{^^JRedefining begin@ExtendedBeam^^J}\def\begin@ExtendedBeam{\@pstrickserr{You can't switch between single and extended beam in custom mode.}\@ehpb}\@oldsinglebegin}%
   \let\@oldextendedbegin\begin@ExtendedBeam
   \def\begin@ExtendedBeam{\message{^^JRedefining begin@SingleBeam^^J}\def\begin@SingleBeam{\@pstrickserr{You can't switch between single and extended beam in custom mode.}\@ehpb}\@oldextendedbegin}%
   #1%
   \end@SpecialObj%
   \fi
\ignorespaces}%
...
\def\begin@DrawbeamObj{%
  ...
  \addto@pscode{%
     ...
     /connectPlaneNodes \ifPst@oe@beamConnectComp true \else false \fi def
     /startvecAbsolute \ifPst@oe@startvecAbsolute true \else false \fi def
  }%
}%
%
...
\def\begin@SingleBeam{%
  \let\@strokesinglebeam\@@strokesinglebeam
}%
\def\begin@ExtendedBeam{%
  \let\@strokeextendedbeam\@@strokeextendedbeam
}%
\def\drawbeam@iii{%
  \begin@SingleBeam
  \ifx\pslinestyle\@none\else
    ...
    \fi
    \addto@pscode{%
       [\ifx\psk@oe@startvec\@empty 0 0 \else \psk@oe@startvec\fi\space 
...
\def\optexp@@@@getcomps#1\@nil{%
  ...
  \else\ifcat a\pst@tempb\relax
    \optexp@getcomp{(\pst@str@basicname@prefix#1)}%
  \else
    \optexp@getcomp{(\pst@str@basicname@prefix\pst@str@basicname@default#1)}%
  \fi\fi%
  \optexp@@getcomps%
}%
...
%  \edef\poe@tmp{\the\poe@cnt}% I don't know how to do it with expansion
  \ifPst@oe@beam
    \ifPst@oe@endbox
      \drawbeam[addtoBeam={beamConnectComp}]{(\optexp@refnodeA)}{\psk@oe@compname}
    \else
      \drawbeam[addtoBeam={beamConnectComp}]{(\optexp@refnodeA)}{\psk@oe@compname}{(\optexp@refnodeB)}
    \fi
  \else
    \ifPst@oe@endbox
      \ifPst@oe@fiberin@
...
\def\crystal@comp{%
   ...
   \psrotate(! @@x \psk@oe@rotate@xref\space mul @@y \psk@oe@rotate@yref\space mul)
      {\psk@oe@angle}{%
...
\def\crystal@lamp{%
  ...
  \edef\@l@s{\psk@oe@lampscale\space}%
  % 
...
\def\polarization@comp{%
  ...
  \ifx\psk@oe@poltype\pst@str@pol@polparallel
     \psline[linestyle=solid, linewidth=\psk@oe@pollinewidth, arrowscale=0.8]{<->}%
  ...
  \ifx\psk@oe@poltype\pst@str@pol@polrcirc
     \psellipticarc[linewidth=\psk@oe@pollinewidth]{->}%
...
%
\def\optgrid@comp{%
   \edef\@g@cnt{\psk@oe@optgridcount\space}%
   \edef\@g@wd{\psk@oe@optgridwidth\space 2 div\space}%
   \edef\@g@ht{\psk@oe@optgridheight\space}%
   \edef\@g@dp{\psk@oe@optgriddepth\space}%
   \edef\@g@step{\psk@oe@optgridwidth\space\@g@cnt div\space}%
