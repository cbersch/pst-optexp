...
\edef\pst@string@detector@round{round}
\edef\pst@string@detector@diode{diode}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% psstyles
%
...
\define@choicekey+[psset]{optexp}{dettype}[\val\nr]{round,diode}%
   {\edef\psk@optexp@detector@type{\val}}
...
%
% Prism
\define@key[psset]{optexp}{prismsize}{\edef\psk@optexp@prism@size{#1}}
\define@key[psset]{optexp}{prismangle}{\edef\psk@optexp@prism@angle{#1}}
%
% Dove Prism
\define@key[psset]{optexp}{doveprismsize}{\edef\psk@optexp@doveprism@size{#1}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
\define@key[psset]{optexp}{extnoden@me}{\edef\psk@optexp@extnode@name{#1}}
%
...
%
\define@key[psset]{optexp}{extnodepostfix}{\edef\psk@optexp@extnode@postfix{#1}}%
%
%
%
%
...
\let\pst@optexp@complist\@empty%
\define@key[psset]{optexp}{compname}{%
   \edef\psk@optexp@compname{#1}%
   \psset{b@sicname=#1}%
   \psset{extnoden@me=#1\psk@optexp@extnode@postfix}%
   % check if compname was already defined
   \ifx\psk@optexp@compname\@empty\else
      \@expandtwoargs\in@{,#1,}{,\pst@optexp@complist,}%
      \ifin@
      ...
      \else
         ...
         \xdef\pst@optexp@complist{\pst@optexp@complist\ifx\pst@optexp@complist\@empty\else,\fi#1}%
         %\XKV@addtolist@x\pst@optexp@complist{#1}%
      \fi
   \fi
}%
...
\def\psoe@cs@out@{}%
\def\psoe@cs@in@{}%
\def\psoe@cs@in@o{\psline[style=Beam](\pst@string@optexp@basicname@default A)(\optexp@nodeA)}%
\def\psoe@cs@out@o{\psline[style=Beam](\optexp@nodeB)(\pst@string@optexp@basicname@default B)}%
\def\psoe@cs@in@f{%
%   \psline[style=FiberOut](\psk@optexp@basicname A)(\optexp@nodeA)
   \pccurve[angleA=0, angleB=180, style=FiberIn](\psk@optexp@basicname A)(\optexp@nodeA)%
}%
\def\psoe@cs@out@f{%
%   \psline[style=FiberOut](\optexp@nodeB)(\psk@optexp@basicname B)
...
\def\psoe@cs@in@i{%
   ...
   \begin@OpenObj
      \addto@pscode{%
         [
         \pst@optexpdict
            (\psk@optexp@basicname) false GetInternalBeamNodes 
            /N@\pst@string@optexp@basicname@default A tx@NodeDict begin load GetCenter end 
         end
      }%
      \psline@ii
}%
\def\psoe@cs@out@i{%
   ...
   \begin@OpenObj
      \addto@pscode{%
         [
         \pst@optexpdict
            /N@\pst@string@optexp@basicname@default B tx@NodeDict begin load GetCenter end 
            (\psk@optexp@basicname) false GetInternalBeamNodes 
         end
      }%
      \psline@ii
...
\def\drawbeam@i#1#2{%
   ...
   \else
      ...
      \edef\psk@optexp@basicnameB{#2\pst@string@optexp@basicname@postfix}%
   \fi
   \if(\pst@tempA
   ...
   \else
      ...
      \edef\psk@optexp@basicnameA{#1\pst@string@optexp@basicname@postfix}%
   \fi
   \let\psk@optexp@basicname\psk@optexp@basicnameB
   \addto@pscode{[ \@nameuse{\psk@connOut}}%
   \let\psk@optexp@basicname\psk@optexp@basicnameA
...
%
% IN
\def\psoe@cs@in@a{tx@NodeDict begin /N@\psk@optexp@basicname 1 load GetCenter end }%
\def\psoe@cs@in@A{\pst@optexpdict (\psk@optexp@basicname) true GetInternalBeamNodes end }%
\def\psoe@cs@in@b{tx@NodeDict begin /N@\psk@optexp@basicname N load GetCenter end }%
\def\psoe@cs@in@B{\pst@optexpdict (\psk@optexp@basicname) false GetInternalBeamNodes end }%
%
% OUT
%
\def\psoe@cs@out@a{tx@NodeDict begin N@\psk@optexp@basicname 1 GetCenter end }%
\def\psoe@cs@out@A{\pst@optexpdict (\psk@optexp@basicname) false GetInternalBeamNodes end }%
\def\psoe@cs@out@b{tx@NodeDict begin N@\psk@optexp@basicname N GetCenter end }%
\def\psoe@cs@out@B{\pst@optexpdict (\psk@optexp@basicname) true GetInternalBeamNodes end }%
%
%
...
\def\end@OptexpObj{%
      \pst@OptexpVerb{(\pst@string@optexp@basicname@default) (\psk@optexp@basicname) CloseOptexpComp}%
  \endgroup
  \ignorespaces%
}%
\def\addafter@par#1{%
...
\def\optdetector@iii{%
   \pnode(tempNode@B){\optexp@nodeA}
   \ifx\psk@optexp@detector@type\pst@string@detector@round
   ...
   \else
      \edef\@sz{\psk@optexp@detector@size\space}%
...
%
\def\doveprism@iii{%
   \edef\@ht{\psk@optexp@doveprism@size\space 0.5 mul\space}%
   \edef\@wd{\psk@optexp@doveprism@size\space 0.9 mul\space}%
...
\def\optprism@iii{%
   % 
   \pst@Verb{%
      /@sz \psk@optexp@prism@size\space def
      /@alpha \psk@optexp@prism@angle\space 0.5 mul def 
...
\def\rightangleprism@iii{%
   % 
   \pst@Verb{%
      /prism@size \psk@optexp@raprism@size\space 0.5 mul def 
