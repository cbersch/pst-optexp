\documentclass{scrartcl}
\usepackage[latin1]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[ngerman]{babel}
\usepackage{lmodern}
\usepackage{pst-optexp}

\begin{document}
\newpsobject{MOLensIn}{lens}{lens=0.5 0.5 0.5}
\newpsobject{MOLensOut}{lens}{lens=2 2 2}
\begin{pspicture}[showgrid=true](5,2) 
   \pnode(0,1){A}\pnode(5,1){B}
   \psset{n=1}
%   \begin{optexp}
   \MOLensIn[abspos=0.5](A)(B){}
%   \optplate[abspos=1](A)(B)
   \MOLensOut[abspos=1](A)(B){}
   \MOLensOut[abspos=4](A)(B){}
   \MOLensIn[abspos=4.5](A)(B){}
   \addtopsstyle{Beam}{fillstyle=solid, fillcolor=red, opacity=0.3}
%   \drawwidebeam[beaminsidefirst, startinside, beamwidth=0.1, beamdiv=5]{1}{2}
   \psset{beaminsidefirst, beaminside, beamangleabs}
   \drawbeam[beaminsidefirst]{1}{2}
%   \custombeam{%
%     \drawwidebeam[beamwidth=0.2, beaminsidelast, beaminsidefirst, beamInside]{(A)}{1}
%     \drawwidebeam[beamdiv=45]{1}{2}
%     \drawwidebeam[stopInside]{2}{3}
%     \drawwidebeam[beamdiv=-45]{3}{4}
%     \drawwidebeam{4}{(B)}
%   }%
% \end{optexp}
\end{pspicture}
\end{document}
